...
describe('GasFeeUtils', () => {
  ...
  it('uses provider gas limit from transaction data when on-chain estimation fails', async () => {
    jest
      .spyOn(EthersUtils, 'getGasLimit')
      .mockRejectedValue(new Error('execution reverted'));
    (EvmLightNodeApi.get as jest.Mock).mockResolvedValue({
    ...
    expect(result.suggested?.gasLimit.toNumber()).toBe(650_000);
  });

  it('uses the higher of provider gas limit and on-chain estimate', async () => {
    jest.spyOn(EthersUtils, 'getGasLimit').mockResolvedValue(400_000);
    (EvmLightNodeApi.get as jest.Mock).mockResolvedValue({
      low: {
        suggestedMaxPriorityFeePerGas: '1',
        suggestedMaxFeePerGas: '20',
