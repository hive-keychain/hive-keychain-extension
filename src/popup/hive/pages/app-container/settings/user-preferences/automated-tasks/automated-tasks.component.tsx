...
const AutomatedTasks = ({
  ...
  const handleSetAutoStake = async (enable: boolean) => {
    ...
    await AutomatedTasksUtils.saveUsernameAutoStake(selectedAccountName, enable);
  };
                ...
                <ComplexeCustomSelect
                  ...
                  options={userTokenOptionList.filter(
                    (userTokenOption) =>
                      !autoStakeTokenList.find(
                        (autoStakeToken) =>
                          autoStakeToken.value.symbol ===
                          userTokenOption.value.symbol,
                      ),
                  )}
            ...
            {autoStakeTokenList.length > 0 && enabledAutoStake && (
              <>
                ...
                <div className="auto-stake-token-list">
                  {autoStakeTokenList.map((autoStakeToken) => (
                    <CustomSelectItemComponent
                      key={`option-${autoStakeToken.label}`}
                      isLast={true}
                      item={autoStakeToken}
                      isSelected={false}
                      handleItemClicked={() => {}}
                      closeDropdown={() => {}}
                      onDelete={() => handleRemoveItem(autoStakeToken)}
                      canDelete={true}
                    />
                  ))}
                </div>
