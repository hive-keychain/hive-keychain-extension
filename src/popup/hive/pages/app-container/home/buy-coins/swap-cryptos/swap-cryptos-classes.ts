  ...
  implements SwapCryptosBaseProviderInterface
{
  ...
  getPairedCurrencyOptionItemList = async (symbol: string) => {
    ...
    if (this.logInfo) {
      console.log('pairedCurrencyRoute', { pairedCurrencyRoute });
    }
  ...
  getMinMaxAmountAccepted = async (from: string, to: string) => {
    ...
    if (this.logInfo) {
      console.log('minMaxRoute', { minMaxRoute, requestHeaders });
    }
    return (
  ...
  getExchangeEstimation = async (amount: string, from: string, to: string) => {
    ...
    if (this.logInfo) {
      console.log('estimationRoute', { estimationRoute, requestConfig, link });
    }
  ...
  implements SwapCryptosBaseProviderInterface
{
  ...
  getMinMaxAmountAccepted = async (from: string, to: string) => {
    const minMaxAcceptedRoute = this.urls.routes.minMaxAccepted;
    if (minMaxAcceptedRoute.trim().length === 0) return [];
    const minMaxRoute = `${this.urls.routes.minMaxAccepted}?api_key=${this.apiKey}&currency_from=${from}&currency_to=${to}`;
    if (this.logInfo) {
      console.log('minMaxRoute', { minMaxRoute });
    }
    return (await axios.get(this.buildUrl(minMaxRoute))).data;
  };
  getExchangeEstimation = async (amount: string, from: string, to: string) => {
    const estimationRoute = `${this.urls.routes.estimation}?api_key=${this.apiKey}&currency_from=${from}&currency_to=${to}&amount=${amount}`;
    ...
    if (this.logInfo) {
      console.log('estimationRoute', { estimationRoute, link });
    }
