...
export const EvmRequestItem = ({
  ...
  const displayWarningIcon = (warnings: EvmTransactionWarning[]) => {
    ...
    const toggleShowWarnings = () => {
      const singleWarning =
        field.warnings?.length === 1 ? field.warnings[0] : undefined;
      if (
        singleWarning &&
        isGroupedSecurityWarning(singleWarning) &&
        hasGroupedSecurityDetails(singleWarning)
      ) {
            ...
            <div className="warning" key={`warning-${index}`}>
              {isGroupedSecurityWarning(warning) ? (
                <GroupedSecurityWarningMessage
                  warning={warning}
                  onIconClick={() => handleOnWarningClicked(index)}
                />
              ) : (
                <>
                  {!warning.ignored && (
